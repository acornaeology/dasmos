...
full v3 expression / macro vocabulary, rendered to JSON and pinned against
a committed golden (``tests/fixtures/golden_schema_v3.json``).
...
and update ``docs/design/json-schema-v3.md`` in the same commit.
"""
...
_GOLDEN = Path(__file__).parent / "fixtures" / "golden_schema_v3.json"
...
def _build_document() -> dict:
    ...
    d.index_base(0x2003, "table")             # populate index_bases
...
def test_golden_is_stamped_v3():
    # A cheap independent guard: the pinned document is v3, and carries the
    # v3 hallmarks (object expressions, a macros section).
    doc = json.loads(_GOLDEN.read_text(encoding="utf-8"))
    assert doc["meta"]["schema_version"] == 3
