...
# Initial ratchet measured when the ADFS-1.30 fixture was first
# vendored. Lower as fidelity gaps close. The dominant residual
# categories at 67:
# - Mid-instruction return-point labels py8dis auto-generates
#   (``return_1``, ``return_10``, …) that dasmos's algorithm names
#   differently or skips.
# - Fred ($&FC..) / Jim ($&FD..) / Sheila ($&FE..) hardware
#   addresses outside acorn_bbc_hardware's current vocabulary
#   (e.g. ``fred_hard_drive_0``).
# - Free-text words from py8dis comment annotations dasmos doesn't
#   yet generate (``baud``, ``ieee``, ``paged``, …).
MAX_COMMENT_TOKENS_DROPPED = 67
