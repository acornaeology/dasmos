; Memory locations
st_ptr_lo       = &00  ; Low byte of the self-test scan pointer. Paired with st_ptr_hi to form a 16-bit indirect pointer walked by self_test_rom_checksum and self_test_ram_pattern. Also used as an anti-aliasing tripwire byte (the INC $00) in ram_test, and as a scratch victim byte during self_test_zp when the zero-page itself is under test.
; &00 referenced 15 times by &e019, &e025, &f031, &f037, &f04e, &f05c, &f072, &f080, &f082, &f08a, &f08c, &f0ad, &f0c7, &f2c7, &f304
st_ptr_hi       = &01  ; High byte of the self-test scan pointer. Paired with st_ptr_lo; incremented page by page as the scanning loops stride through each 256-byte block of ROM or RAM. Also a scratch victim byte during self_test_zp.
; &01 referenced 15 times by &f033, &f03b, &f056, &f061, &f076, &f095, &f0a2, &f0b3, &f0bc, &f0cf, &f2c9, &f2ee, &f2f4, &f300, &f306
st_page_count   = &02  ; Page counter for the self-test scans. Pre-loaded with a hard-coded &20 (32 pages = 8 KiB) at every site and decremented once per page by self_test_rom_checksum, self_test_ram_pattern, and both phases of self_test_ram_incr. The self-test therefore always covers exactly the low 8 KiB of RAM (&0000-&1FFF) and the 8 KiB ROM (&E000-&FFFF), regardless of what the boot-time ram_test found: this counter is never loaded from top_ram_page. Also a scratch victim byte during self_test_zp.
; &02 referenced 10 times by &f035, &f03f, &f052, &f063, &f07a, &f097, &f0a6, &f0b6, &f0c0, &f0d2
st_pass_phase   = &03  ; Self-test pass-phase flag. Initialised by self_test at entry and toggled by self_test_pass_done at the end of each full pass; bit 7 then selects between a plain restart from self_test_reset_adlcs and the alternate path through self_test_alt_pass, which drives the ADLC control registers slightly differently to catch intermittent faults.
; &03 referenced 3 times by &f003, &f264, &f268
mem_ptr_lo      = &80  ; Low byte of the indirect pointer. Paired with mem_ptr_hi. Used by ram_test to scan memory upward one page at a time, and by the TX/RX byte loops in the frame handlers, e.g. transmit_frame_a.
; &80 referenced 23 times by &e00d, &e017, &e01b, &e023, &e027, &e248, &e3c9, &e486, &e4b9, &e4d9, &e4df, &e501, &e510, &e530, &e536, &e558, &e567, &e59e, &e5a4, &e5cc, &e62f, &e635, &e65d
mem_ptr_hi      = &81  ; High byte of the indirect pointer. Paired with mem_ptr_lo.
; &81 referenced 23 times by &e011, &e013, &e02d, &e02f, &e049, &e24c, &e3cd, &e48a, &e4bd, &e4e7, &e4ee, &e514, &e53e, &e545, &e56b, &e5a9, &e5ab, &e5f5, &e5fc, &e63a, &e63c, &e686, &e68d
top_ram_page    = &82  ; Highest page that verified &AA/&55 during ram_test. Used later by workspace initialisation to decide how much RAM is safe to touch.
; &82 referenced 3 times by &e031, &e5ad, &e63e
...
; &0200 referenced 6 times by &e47c, &e4af, &e4e9, &e540, &e5d3, &e664
tx_end_hi       = &0201  ; High byte of the TX end-pointer. Paired with tx_end_lo.
; &0201 referenced 6 times by &e481, &e4b4, &e4f0, &e547, &e5f7, &e688
...
; &0214 referenced 7 times by &e1a9, &e32a, &e452, &e692, &e6bf, &e6de, &e70b
ctr24_mid       = &0215  ; Middle byte of the 24-bit counter. Rooted at ctr24_lo.
; &0215 referenced 4 times by &e695, &e6c4, &e6e1, &e710
ctr24_hi        = &0216  ; High byte of the 24-bit counter. Rooted at ctr24_lo.
; &0216 referenced 4 times by &e69a, &e6c9, &e6e6, &e715
...
; &0228 referenced 12 times by &e14a, &e1fa, &e205, &e208, &e20e, &e22c, &e2cb, &e37b, &e386, &e389, &e38f, &e3ad
announce_flag   = &0229  ; Event-driven re-announcement selector. Set by rx_a_handle_80 / rx_b_handle_80 to &40 (side A selected) or &80 (side B) when a peer bridge's BridgeReset scout arrives. Cleared in re_announce_done once all ten BridgeReply frames have been emitted. Polled by main_loop_idle; the entire re-announcement state machine is quiescent while this byte is zero.
; &0229 referenced 6 times by &e035, &e089, &e0a0, &e0c4, &e1eb, &e36c
...
; &022c referenced 4 times by &e0b0, &e0db, &e1e6, &e367
...
; &0249 referenced 4 times by &e195, &e1c7, &e316, &e348
reachable_via_b = &025a  ; 256-byte routing table for forwarding out of side B. Indexed by destination network number; a non-zero entry means "this network is reachable from here". Consulted by rx_frame_a. Initialised with net_num_b (our own side-B network, directly reachable) and 255 (broadcast), then extended by bridge-announcement messages received on side A.
; &025a used as index base 7 times by &e0fc, &e198, &e377, &e428, &e43c, &e441, &e5e6
reachable_via_a = &035a  ; 256-byte routing table for forwarding out of side A. Indexed by destination network number; a non-zero entry means "this network is reachable from here". Consulted by rx_frame_b. Initialised with net_num_a (our own side-A network, directly reachable) and 255 (broadcast), then extended by bridge-announcement messages received on side B.
; &035a used as index base 7 times by &e1f6, &e27d, &e319, &e42b, &e436, &e444, &e677
...
; &045a referenced 4 times by &e45a, &e490, &e580, &e611
tx_dst_net      = &045b  ; TX frame buffer byte 1 – destination network number.
; &045b referenced 8 times by &e45d, &e495, &e58e, &e5e1, &e5f2, &e61f, &e672, &e683
tx_src_stn      = &045c  ; TX frame buffer byte 2 – source station number.
; &045c referenced 2 times by &e462, &e49a
tx_src_net      = &045d  ; TX frame buffer byte 3 – source network number.
; &045d referenced 10 times by &e1a6, &e1be, &e327, &e33f, &e465, &e49d, &e5d6, &e5de, &e667, &e66f
...
; &0460 referenced 3 times by &e044, &e0cd, &e475
net_num_a       = &c000  ; Econet side A network number, read at &C000. Sourced from a bank of jumpered links buffered by a 74LS244. 7-bit (range 1-127); the top link is always made, so bit 7 reads 0. The Bridge has no station number of its own – it sits on each Econet segment as a promiscuous receiver and broadcaster.
; &c000 referenced 13 times by &e041, &e0ca, &e156, &e1c1, &e1ff, &e2dd, &e324, &e33c, &e35a, &e433, &e5db, &e67c, &f24c
adlc_a_cr1      = &c800  ; ADLC A control/status port 0. Write: CR1 (or CR3 when AC=1 in CR1). Read: SR1. IRQ summary is in SR1 bit 7; wait_adlc_a_irq spins on it.
; &c800 referenced 32 times by &e05a, &e05f, &e081, &e0d2, &e122, &e141, &e178, &e39a, &e3e4, &e3f2, &e401, &e51e, &e526, &e553, &e570, &e5b8, &e703, &e708, &e724, &f007, &f021, &f0d8, &f10c, &f11e, &f128, &f15f, &f1ad, &f1b5, &f271, &f27b, &f28e, &f2cd
adlc_a_cr2      = &c801  ; ADLC A control/status port 1. Write: CR2 (or CR4 when AC=1). Read: SR2.
; &c801 referenced 39 times by &e051, &e064, &e0e4, &e0f2, &e109, &e127, &e12c, &e3c1, &e3fc, &e406, &e519, &e55f, &e578, &e586, &e596, &e5bd, &e5c2, &e6e9, &e6f0, &e6f5, &e6fa, &e71f, &f017, &f029, &f0df, &f0e6, &f119, &f186, &f1df, &f1ed, &f210, &f22d, &f247, &f276, &f283, &f293, &f2a2, &f2d2, &f2e1
adlc_a_tx       = &c802  ; ADLC A TX/RX FIFO port. Write: push a byte into the TX FIFO (frame continues). Read: pop a byte from the RX FIFO.
; &c802 referenced 26 times by &e0e9, &e0f7, &e10e, &e115, &e133, &e3a2, &e3a9, &e3bc, &e532, &e538, &e55a, &e57d, &e58b, &e59b, &e5a1, &e5c9, &f12d, &f131, &f164, &f168, &f1e4, &f1f7, &f215, &f21b, &f232, &f238
...
; &c803 referenced 2 times by &e3f7, &f00f
net_num_b       = &d000  ; Econet side B network number, read at &D000. Sourced from a bank of jumpered links buffered by a 74LS244. 7-bit (range 1-127); the top link is always made, so bit 7 reads 0. Paired with net_num_a on the opposite segment.
; &d000 referenced 12 times by &e15c, &e1a3, &e1bb, &e1d9, &e2d7, &e342, &e380, &e439, &e472, &e5eb, &e66c, &f258
adlc_b_cr1      = &d800  ; ADLC B control/status port 0. Write: CR1 (or CR3 when AC=1 in CR1). Read: SR1. IRQ summary is in SR1 bit 7; wait_adlc_b_irq spins on it. CR3 bit 7 (reached via this port with AC=1) drives IC18's ~LOC/DTR pin, which sources current to the front-panel status LED (anode tied to the pin, cathode to ground). The MC6854 inverts the bit when driving the pin, so CR3 bit 7 = 0 puts the pin HIGH and the LED LIT; CR3 bit 7 = 1 puts the pin LOW and the LED DARK. adlc_b_full_reset writes CR3 = &00 (LED on – the steady normal-operation state); self_test_reset_adlcs writes CR3 = &80 (LED off – marks the start of a self-test pass).
; &d800 referenced 29 times by &e070, &e073, &e079, &e0a7, &e219, &e2a3, &e2c2, &e2f9, &e3ea, &e40c, &e41b, &e4c7, &e4cf, &e4fc, &e601, &e649, &e6b7, &e6bc, &e6d8, &f00a, &f024, &f0ed, &f10f, &f114, &f1b0, &f1bf, &f1c9, &f200, &f27e
adlc_b_cr2      = &d801  ; ADLC B control/status port 1. Write: CR2 (or CR4 when AC=1). Read: SR2.
; &d801 referenced 34 times by &e067, &e076, &e240, &e265, &e273, &e28a, &e2a8, &e2ad, &e416, &e420, &e4c2, &e508, &e609, &e617, &e627, &e64e, &e653, &e69d, &e6a4, &e6a9, &e6ae, &e6d3, &f01c, &f02c, &f0f4, &f0fb, &f13e, &f14c, &f16f, &f18c, &f1a6, &f1ba, &f227, &f286
adlc_b_tx       = &d802  ; ADLC B TX/RX FIFO port. Write: push a byte into the TX FIFO (frame continues). Read: pop a byte from the RX FIFO.
; &d802 referenced 26 times by &e221, &e228, &e23b, &e26a, &e278, &e28f, &e296, &e2b4, &e4db, &e4e1, &e503, &e60e, &e61c, &e62c, &e632, &e65a, &f143, &f156, &f174, &f17a, &f191, &f197, &f1ce, &f1d2, &f205, &f209
...
; &d803 referenced 2 times by &e411, &f012


    org &e000

.pydis_start
; &e000 used as index base 7 times by &f2a9, &f2ac, &f2af, &f2b2, &f2b5, &f2b8, &f2bb
.reset
...
;    read-modify-write that drives the data bus three times with values unrelated to the
;    test pattern (the cycle-4 dummy write is a classic NMOS 6502 quirk that is exploited
;    here), clobbering any residue of &AA or &55.
...
;
; Falls through to main_loop on success. A clean reset therefore emits exactly two frames
; before steady-state polling begins. See docs/analysis/two-broadcasts-one-template.md
...
;
; The Bridge's continuous-operation entry point. Reached by fall- through from the reset
; handler once startup completes, and by JMP from fourteen other sites – every routine
; that takes an "escape to main" path (wait_adlc_a_idle,
; transmit_frame_a/transmit_frame_b, etc.) lands here, so main_loop is the anchor of
; every packet-processing cycle.
;
; The header (&E051-&E078) forces each ADLC into a known RX-listening state: if SR2 bit 0
; or 7 (AP or RDA) is already set from a partial or aborted previous operation, CR1 is
; cycled through &C2 (reset TX, leave RX running) before setting it to &82 (TX in reset,
; RX IRQs enabled). CR2 is set to &67 – the standard listen-mode value used throughout
; the firmware.
;
...
;
; The re-announce path (re_announce) rebuilds the announcement frame, sets tx_ctrl to &81
; (distinguishing it from the reset-time &80 first announcement), then dispatches to side
; A or side B based on announce_flag bit 7. The timer is re-armed to &8000 (32768 idle
; iterations) after each announce, giving a roughly constant cadence regardless of how
; busy the ADLCs are with other traffic.
...
;
; Each invocation decrements announce_count. When it hits zero, announce_flag is cleared
; (re_announce_done); the burst is complete and the idle path goes quiet until another
; BridgeReset arrives. Otherwise the timer is re-armed to &8000 and control returns to
...
;    network), jump to rx_a_not_for_us (&E13F): ignore the frame, re-listen, drop back to
;    main_loop_poll without a full main_loop re-init.
; 2. Drain. Read the rest of the frame in byte-pairs into &023C+Y up to Y=20 (the Bridge
;    only keeps the first 20 bytes). After the drain, force CR1=0 and CR2=&84 to halt the
;    chip and test SR2 bit 1 (FV, Frame Valid). If FV is clear, the frame was corrupt or
;    short – bail to main_loop. If SR2 bit 7 (RDA) is also set, read one trailing byte.
; 3. Broadcast check. Only frames with dst_stn == dst_net == &FF (full broadcast) proceed
;    to the bridge-protocol dispatcher. Everything else falls to rx_a_forward (&E208),
;    the cross-network forwarding path.
...
;
; Called when a received frame on side A is broadcast + port=&9C + ctrl=&83 – the querier
; is asking "can you reach network X?", where X is the byte at offset 13 of the payload
; (rx_query_net).
;
; Consults reachable_via_b[rx_query_net]. If the entry is zero, there is no route to that
; network so the query is silently dropped (JMP main_loop via rx_a_query_done). If
; non-zero, falls through to the shared response body at rx_a_handle_82 to transmit the
; reply – so the targeted query is effectively the general query with an up-front routing
; filter.
.rx_a_handle_83
...
;
; The response is a complete four-way handshake transaction, which the Bridge drives from
; the responder side as two transmissions (scout, then data) with an inbound ACK after
; each:
;
; 1. Build a reply-scout template via build_query_response, addressed back to the querier
;    on its local network with tx_src_net patched to our net_num_b.
...
;
;    The answer thus consists of the dst/src quad plus two payload bytes, packed into the
;    smallest Econet frame that can carry it.
...
;
; Called when a received frame on side A is broadcast + port=&9C + ctrl=&80 – a bridge on
; the far side is advertising a fresh topology, likely because it has itself just come
; up. The handler:
;
...
;    BridgeReset and BridgeReply) to mark the sender's known networks as reachable-via-A.
;
; This is one of only two places in the ROM that sets announce_flag non-zero (the other
; is the mirror rx_b_handle_80). Receiving a BridgeReply (ctrl=&81) does not trigger the
; burst; only receiving a BridgeReset does. A solo bridge therefore stays silent after
; its boot-time BridgeReset pair, because nothing comes back to trigger a response. See
; docs/analysis/event-driven-reannouncement.md.
...
; BridgeReset) or via fall-through from rx_a_handle_80 (which additionally wipes routing
; state before the learn loop).
;
...
;
; After the learn loop, append our own net_num_a to the payload and bump rx_len. Falling
; through to rx_a_forward re-broadcasts the augmented frame out of ADLC B, so any bridges
; beyond us on that side hear about the announced networks plus us as one further hop
; along the route. This is classic distance-vector flooding.
...
; - Fall-through from rx_a_handle_81 (at &E207): we've learned from the announcement and
...
; bridged four-way handshake when the initial scout came from side A: two frames travel A
; → B (scout and data) and two travel B → A (two ACKs).
...
; net_num_a, transmit via ADLC B, tx_src_net patched to net_num_a, response-data's first
; payload byte (at the tx_ctrl slot) encodes net_num_b. See rx_a_handle_82 for the full
...
; net_num_b), set announce_count = 10 and announce_flag = &80 (bit 7 set = next outbound
; on side B). Falls through to rx_b_handle_81.
;
...
; network number reachable via side B, marks reachable_via_b[x] = &FF for each (mirror of
; the A-side writing reachable_via_a). Appends net_num_b to the payload and falls through
; to rx_b_forward for re-broadcast onto side A.
...
;   but has a control byte outside the recognised bridge-protocol set (&80-&83).
; - Fall-through from rx_b_handle_81 (at &E387): we've learned from the announcement and
;   appended net_num_b to the payload; now propagate it onward.
;
; | stage | direction | drain                                | retransmit                               |
; |-------|-----------|--------------------------------------|------------------------------------------|
...
;
; Spin reading SR1 of ADLC A until the IRQ bit (bit 7) is set. Called from 19 sites where
; the code needs to wait for the ADLC to signal an event (frame complete, RX data
; available, TX ready, etc.).
;
; The Bridge does not route the ADLC ~IRQ output to the 6502 ~IRQ line (that pin is used
; for the self-test push-button), so ADLC attention is obtained by polling.
...
;
; Everything else starts at zero and is populated later by bridge-protocol announcements
; learned in the rx handlers (see rx_a_handle_80 / rx_b_handle_80).
;
...
;
; A calibrated busy-wait used by the query-response paths to stagger their transmissions.
; Called from rx_a_handle_82 (at &E1AC) and rx_b_handle_82 (at &E32D), in each case with
; ctr24_lo pre-loaded with the bridge's opposite-side network number (net_num_b for
...
; transmitted via ADLC A first (announcing "network net_num_b is reachable through me" to
; side A's stations), then tx_data0 is patched to net_num_a and the same frame is
...
;
; The src_stn/src_net fields are both set to the constant &18. The Bridge has no station
; number of its own (only network numbers, per the Installation Guide), so these fields
; are not real addresses. Receivers do not use them for routing -- rx_a_handle_81 reads
; the payload starting at offset 6 and ignores bytes 2-3 entirely. The most plausible
; role for &18 is defensive redundancy: together with dst=(&FF,&FF), ctrl=&80/&81 and
; port=&9C it gives a receiver multiple ways to confirm that a received frame is a
; well-formed bridge announcement.
;
; Also writes &06 to tx_end_lo and &04 to tx_end_hi (so the transmit routine sends bytes
; &045A..&0460 inclusive – 7 bytes when X=1), loads X=1 (trailing-byte flag for
...
;
; A second frame-builder (sibling of build_announce_b) used by the bridge-query response
; path. Called twice per response: once to build the reply scout (ctrl=&80 + reply_port
; as the port), then after the querier's scout-ACK has been received, called again to
; rebuild the buffer as a data frame – the caller then patches bytes 4 and 5 (labelled
; tx_ctrl and tx_port but genuinely payload in a data frame) with the routing answer.
; Where build_announce_b writes a broadcast-addressed template, this one builds a unicast
; reply:
;
; | field      | value         | notes                               |
; |------------|---------------|-------------------------------------|
...
;
; Called from the two query-response paths (&E1A0 and &E1B8 on side A; &E321 and &E339 on
; side B). Each caller then patches a subset of the fields before calling
...
; control block &045A) through ADLC A's TX FIFO. Termination is controlled by the 16-bit
; pointer tx_end_lo/tx_end_hi (&0200/&0201): the loop sends byte pairs until mem_ptr + Y
; reaches or passes (tx_end_hi:tx_end_lo). X is a flag – non-zero means send one extra
; trailing byte after the terminator (used by builders that append a payload like
; build_announce_b's net_num_b at &0460).
;
; On entry:
;
; | register / byte | role                                             |
...
;
; Abnormal exit: if any of the three wait_adlc_a_irq polls returns with SR1's V-bit clear
; instead of set (meaning the ADLC didn't reach the expected TDRA state), the routine
; drops the caller's return address from the stack and JMP's into main_loop – the same
; escape-to-main pattern used by wait_adlc_a_idle.
;
...
;
; The drain is capped at top_ram_page (set by the boot RAM test) so very long frames fill
; available RAM and no further.
;
...
; - If tx_dst_net equals net_num_b (our own B-side network), normalise it to zero – from
;   side B's perspective the frame is now "local".
;
; On any of the "reject" paths above, and on any sub-step that fails (no AP/RDA, no Frame
; Valid, no response at all), takes the standard escape-to-main-loop exit: PLA/PLA/JMP
; main_loop.
;
...
; - If tx_dst_net equals net_num_a (our own A-side network), normalise it to zero – from
;   side A's perspective the frame is now "local".
;
; On any of the "reject" paths above, and on any sub-step that fails (no AP/RDA, no Frame
; Valid, no response at all), takes the standard escape-to-main-loop exit: PLA/PLA/JMP
; main_loop.
...
;
; Pre-transmit carrier-sense: polls ADLC A's SR2 until the Rx Idle bit goes high (SR2 bit
; 2 = 15+ consecutive 1s received, i.e. the line is quiet and it is safe to start a
; frame). A 24-bit timeout counter at ctr24_lo/mid/hi (&0214-&0216) starts at &00_00_FE
; and increments LSB-first; overflow takes ~131K iterations, a few seconds at typical bus
; speeds.
;
; Each iteration re-primes CR2 with &67 (clear TX/RX status, FC_TDRA, 2/1-byte, PSE) then
; reads SR2. Three outcomes:
;
...
;   off by cycling CR1 through &C2 → &82 (reset TX without touching RX) and keep polling.
...
;   caller's saved return address from the stack and JMP main_loop escapes into the main
;   Bridge loop. The code between the caller's JSR and the main loop is skipped entirely.
;   See docs/analysis/escape-to-main-control-flow.md.
;
...
.rom_body_gap
    for _dasmos_fill%, 1, 2264 : equb &ff : next                      ; e728: ff ff ff... ......
; ***************************************************************************************
...
; reconfigures the ADLCs and drives their control registers in ways that will disturb any
; in-flight frames.
;
; The official self-test rig is a minimal Econet segment, not a full one. It comprises a
; clock box (which on the original Acorn/SJ design provides only the bit-clock, not
; biasing), a T-piece joining both Bridge ports to one cable to the clock box, and a
; single terminator on the clock box's other socket. The terminator does double duty: it
; terminates the line and provides the DC bias that lets the differential pair carry data
; at all. One terminator is enough to bias the line for the loopback test to work, which
; is why the rig has just one – a real two-station Econet segment would have a terminator
; at each end of the cable run. A bare port-to-port patch cable will not work: without
; clock and without bias the loopback tests can never see a frame. See
...
; CR3 is programmed to &80 (bit 7 set) instead of &00. CR3 bit 7 is the MC6854's LOC/DTR
; control bit, and the pin it drives is inverted relative to the bit – when the control
; bit is HIGH, the pin output goes LOW. On ADLC B (IC18) that pin sources current through
; the front-panel status LED (anode tied to the pin, cathode to ground), so CR3 bit 7 = 1
; leaves the LED dark. The LED is therefore extinguished at the start of every normal
; self-test pass, marking the transition out of normal-operation steady-on. ADLC A's
; LOC/DTR pin is not wired and gets the same write for code symmetry only.
;
...
; during a healthy self-test is the alternation between this dark "early-tests" phase and
; the lit "loopback-and-after" phase, modulated by the two-pass structure described at
; self_test_pass_done.
;
; Re-entered at &F26C after each "normal" pass completes; the alternate path at
; self_test_alt_pass deliberately skips this routine on every other pass, leaving B's CR3
; bit 7 cleared from the previous loopback reset (so the alt-pass runs entirely in the
; LED-lit state).
...
;
; Writes &55 to &00, &01, &02 and reads them back; then &AA and reads back. Failure jumps
; to ram_test_fail (via self_test_ram_fail_jump, because RAM isn't trustworthy enough at
; this point to use a countable blink).
;
...
; KiB). The required total is balanced on-disc by the rom_checksum_adjust byte at &FFF0,
; hand-tuned to make the ROM-wide sum land on &55.
...
; self-test workspace at &00/&01/&02), iterates through the full 8 KiB of RAM and checks
; that each byte can store both &55 and &AA. Pointer in (&00,&01) = &0000, Y starts at 4
; and wraps, page count in &02 = &20 (32 pages = 8 KiB).
;
; On mismatch, jumps to ram_test_fail (&F28C) – a different failure handler from
; self_test_fail, because a broken RAM cannot use the normal blink-code loop which needs
; RAM workspace.
...
; cycles through 0..&FF and then reinitialised each page with a different offset, giving
; a distinctive pattern across the RAM that catches address-line faults). Then reads back
; and verifies.
;
; Catches failures that a plain &55/&AA pattern would miss: particularly address-line
; shorts, where writing to (say) &0410 and &0420 would land at the same cell and produce
; the same bytes under a uniform pattern but different bytes under this one.
;
...
;
; Checks that both ADLCs show the expected register state after self_test_reset_adlcs has
; configured them. Tests specific bits of SR1 and SR2 on each chip (ADLC A bits from
...
; across both Bridge ports, single terminator on the clock box's other socket (which both
; terminates the line and provides its DC bias). See self_test for the full rationale.
; Reconfigures ADLC A for transmit (CR1=&44) and ADLC B for receive (CR1=&82), then sends
; a 256-byte sequence (0,1,2,…,255) out of A and verifies each byte is received on B in
; order by incrementing X alongside the sender's Y.
;
; Four phases:
;
; 1. Pre-fill the A TX FIFO with bytes 0-7 (Y=0..7) while B is still settling – priming
;    the pipeline before any RX checks begin.
; 2. Handshake opening: wait for B's first RX IRQ, verify AP, read and match bytes 0 and
;    1. This is the special "opening" case because the AP/RDA transitions happen on the
...
;    end-of-frame marker. Drain the remaining bytes on B (another 255 iterations to empty
;    B's FIFO), then wait for the Frame Valid bit to confirm a clean end-of-frame.
;
; Every mismatch or missing status bit jumps to the shared fail target at
; loopback_a_to_b_fail (&F151) which loads code 5 and hands off to self_test_fail. Falls
; through to self_test_loopback_b_to_a on success.
...
;
; Mirror of self_test_loopback_a_to_b with adlc_a_* and adlc_b_* swapped: ADLC B becomes
; transmitter (CR1=&44), ADLC A the receiver (CR1=&82), and the same 256-byte sequence is
; sent and verified. Fail target loads code 6 (instead of 5). See
...
; loopback-test configuration: the jumpers on the bridge board should be set for 1 and 2
; respectively before the self-test button is pressed, so that the network numbers are
; predictable and the loopback tests can complete without colliding with anything else a
; tester might leave plugged in.
;
...
;
; Reached when every test in a pass has succeeded. The self-test doesn't stop – it loops
; indefinitely until reset. Toggles bit 7 of &0003 (the self-test scratch byte) via EOR
; #&FF; if bit 7 is set after the toggle, JMPs to self_test_reset_adlcs for another full
; pass. Otherwise falls through to self_test_alt_pass, which resets ADLCs differently
; before re-entering the ZP test.
;
...
; ROM-checksum, and RAM tests. The first loopback test then issues CR1 = &C0 on B, which
; – per the MC6854 datasheet – clears the LOC/DTR control bit, so the LED comes on for
; the rest of the pass. The alt-pass runs the same tests over again but skips
...
; broken, reading/writing RAM is exactly what's untrustworthy), so it generates its blink
; pattern from ROM-based DEC abs,X instructions that exercise the CPU for timing without
; touching RAM.
;
...
;
; Common failure exit for every non-RAM self-test stage. Called with the error code in A.
; Saves two copies of the code in &00/&01 then enters an infinite loop that blinks the
; LED (via CR3 bit 7 on ADLC B, which is the pin that drives the front-panel LED) a count
...
;
; Code 1 is not used: the zero-page integrity test's failure path routes to ram_test_fail
; via self_test_ram_fail_jump (&F09D), not here, because any failure of the first three
; RAM tests means normal counting loops can't be trusted. ram_test_fail (&F28C) uses a
...
; LED-on followed by ~1 s of LED-off, so a burst of N pulses contains N visible flashes.
; After the burst, the LED is left in its OFF state (the last CR3 = &80 write before the
; spacer runs) and the spacer's 8× DEX/DEY iteration extends that OFF state to ~8 s
; before the next burst begins. The operator counts the LED-on flashes inside each burst
; – distinguishing them from the long ~8 s LED-off gap that separates bursts – to
...
.self_test_fail_spacer_delay
    ...
    for _dasmos_fill%, 1, 3301 : equb &ff : next                      ; f30b: ff ff ff... ......
...
.rom_vector_gap
    for _dasmos_fill%, 1, 9 : equb &ff : next                         ; fff1: ff ff ff... ......
; ***************************************************************************************
...
; rather than a real handler. RESET enters the main firmware at &E000. IRQ/BRK – the line
; the self-test push-button pulls low – enters at &F000; pressing the button therefore
; runs the self-test, and any BRK instruction anywhere in the ROM would do the same
; thing.
.hardware_vectors
