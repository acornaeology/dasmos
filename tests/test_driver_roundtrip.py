...
class TestAutoGeneratedLabels:
    ...
    def test_unnamed_data_target_gets_l_prefix(
        self, roundtrip_via_beebasm,
    ):
        # &8004 holds a data byte the LDA at &8000 reads; no explicit
        # label, so the renderer synthesises ``l8004`` and uses it as
        # the operand symbol.
        source = """
            org &8000
        .start
        ...
        assert "lda l8004" in text
        # Label gets emitted inline at &8004.
        assert ".l8004" in text

    def test_unnamed_code_target_gets_c_prefix(
        self, roundtrip_via_beebasm,
    ):
        # &8003 is the target of a forward branch (BNE) — code, single
        # forward reference, so the heuristic picks ``c<addr>``.
        source = """
            org &8000
        .start
            lda #&00
        ...
        text = roundtrip_via_beebasm(source, 0x8000, configure)
        # The branch operand resolves to the synthesised c<addr> name.
        assert ".c8005" in text
        assert "bne c8005" in text

    def test_jsr_only_target_gets_sub_c_prefix(
        self, roundtrip_via_beebasm,
    ):
        # &8004 is reached only via JSR — so the heuristic picks the
        # ``sub_c<addr>`` form.
        source = """
            org &8000
        .start
        ...
        def configure(d):
            d.entry(0x8000, name="start")
            # No explicit name for &8007 so auto-labels fire.

        text = roundtrip_via_beebasm(source, 0x8000, configure)
        assert ".sub_c8007" in text
        assert "jsr sub_c8007" in text

    def test_backward_branch_target_gets_loop_c_prefix(
        self, roundtrip_via_beebasm,
    ):
        # A single backward branch (BNE) loops back to a target a few
        # bytes earlier — heuristic picks the ``loop_c<addr>`` form.
        source = """
            org &8000
        .start
            ldx #&05
        ...
        text = roundtrip_via_beebasm(source, 0x8000, configure)
        assert ".loop_c8002" in text
        assert "bne loop_c8002" in text

    def test_explicit_label_takes_precedence_over_auto(
        self, roundtrip_via_beebasm,
    ):
        # Even though &8003 is JSR'd to and would otherwise get
        # ``sub_c8003``, the explicit ``d.label`` registration wins.
        source = """
            org &8000
        .start
            jsr helper
    ...
    def test_footer_lists_generated_names(
        ...
        assert "sub_c8007" in footer
        assert "l8008" in footer
    ...
    def test_configurable_prefixes(self, tmp_path, assemble_beebasm):
        ...
        # with the default ``l`` / ``c`` / ``sub_c`` / ``loop_c``.
        # Prefixes live on the Disassembler — they're a content
        # decision (analysis), not a rendering one.
        from dasmos.disassembler import Disassembler
        ...
        # The default ``sub_c`` form does NOT appear.
        assert "sub_c" not in text
